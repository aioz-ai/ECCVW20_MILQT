...
def _load_dataset(dataroot, name, img_id2val, label2ans):
    ...
    anotation_path = os.path.join(
        dataroot, 'v2_mscoco_%s_annotations.json' % \
                  (name + '2014' if 'test' != name[:4] else name))
    ...
    if 'test'!=name[:4]: # train, val
        ...
        for question, answer, anotation in zip(questions, answers, anotations):
            ...
            #map onehot
            map_anotation = ['yes/no', 'number', 'other']
            anotation_onehot = (1.0*(np.array(map_anotation)== anotation['answer_type'])).tolist()
        ...
        for question in questions:
            ...
            if not COUNTING_ONLY or is_howmany(question['question'], None, None):
                entries.append(_create_entry(img_id2val[img_id], question, None, torch.tensor([0.0,0.0,0.0])))

    return entries

def _load_visualgenome(dataroot, name, img_id2val, label2ans, adaptive=True, q_type_map_link = 'question_type_mapping_VQA.txt'):
    """Load entries
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, args, dictionary, dataroot='data', adaptive=False, max_boxes=100, question_len=12):
        ...
        self.num_qts_candidates = 3 #number of answer type class
        ...
        if args.use_MoD:
            print("Using MoD features")
            h5_path = os.path.join(args.MoD_dir , '%s%s.hdf5' % (name, '' if self.adaptive else '36'))
        else:
