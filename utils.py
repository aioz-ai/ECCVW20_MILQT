...
def tfidf_loading(use_tfidf, w_emb, args):
    ...
    if use_tfidf:
        if args.use_TDIUC:
            dict = dataset_TDIUC.Dictionary.load_from_file(os.path.join(args.TDIUC_dir, 'dictionary.pkl'))
        else:
            dict = dataset_VQA.Dictionary.load_from_file('data/dictionary.pkl')

        # load extracted tfidf and weights from file for saving loading time
        if args.use_TDIUC:
            if os.path.isfile(os.path.join(args.TDIUC_dir, 'embed_tfidf_weights.pkl')) == True:
                print("Loading embedding tfidf and weights from file")
                with open(os.path.join(args.TDIUC_dir ,'embed_tfidf_weights.pkl') , 'rb') as f:
                ...
                tfidf, weights = dataset_TDIUC.tfidf_from_questions(['train', 'val', 'test2015'], args, dict)
                w_emb.init_embedding(os.path.join(args.TDIUC_dir, 'glove6b_init_300d.npy'), tfidf, weights)
                with open(os.path.join(args.TDIUC_dir, 'embed_tfidf_weights.pkl'), 'wb') as f:
            ...
            if os.path.isfile('data/embed_tfidf_weights.pkl') == True:
                ...
                print("Embedding tfidf and weights haven't been saving before")
                tfidf, weights = dataset_VQA.tfidf_from_questions(['train', 'val', 'test2015'], dict)
